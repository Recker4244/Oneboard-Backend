...
describe('Team Controller', () => {
  describe('addMember', () => {
    it('should add a member to the project', async () => {
      jest.spyOn(services, 'addMember').mockResolvedValue([
        { project_id: 'abc', key_status: true, username: 'Balkar', role: 'Engineer 1', start_date: '2020-01-01', end_date: '2020-01-01', cost: 2000 },
      ]);
      const req = {
        params: { id: 'abc' },
        body: { team_members: [{ username: 'Balkar', role: 'Engineer 1', key_status: true, start_date: '2020-01-01', end_date: '2020-01-01', cost: 200 }] },
      };
      const res = { status: jest.fn().mockReturnThis(), json: jest.fn() };
      await controller.addMember(req, res);
      expect(res.status).toHaveBeenCalledWith(201);
      expect(res.json).toHaveBeenCalledWith([
        { project_id: 'abc', username: 'Balkar', key_status: true, role: 'Engineer 1', start_date: '2020-01-01', end_date: '2020-01-01', cost: 2000 },
      ]);
    });
  ...
  describe('getTeam', () => {
    it('should get the team of the project', async () => {
      ...
      const req = { params: { project_id: 'abc' } };
      const res = { status: jest.fn().mockReturnThis(), json: jest.fn() };
